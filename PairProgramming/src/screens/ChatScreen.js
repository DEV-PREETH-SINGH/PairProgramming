...
const ChatScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        ...
        const response = await axios.get('http://192.168.68.77:5000/api/messages/get-messages', {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://192.168.68.77:5000/api/messages/send-message', {
        senderUID: currentUserUID,
        receiverUID: extractedOtherUserUID,
        message: messageText,
      });
